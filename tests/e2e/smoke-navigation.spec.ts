...
test.describe('Smoke Tests - Navigation @smoke', () => {
  ...
  test('Vehiculos page loads', async ({ page }) => {
    ...
    await expect(
      page.locator(
        '[data-testid="vehiculos-list"], [data-testid="empty-state"]'
      )
    ).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Depositos page loads', async ({ page }) => {
    ...
    await expect(
      page.locator(
        '[data-testid="depositos-list"], [data-testid="empty-state"]'
      )
    ).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Inversores page loads', async ({ page }) => {
    ...
    await expect(
      page.locator(
        '[data-testid="inversores-list"], [data-testid="empty-state"]'
      )
    ).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Navigation links work', async ({ page }) => {
    ...
    const navLinks = [
      {
        selector: '[data-testid="nav-clientes"], a[href*="clientes"]',
        url: '/clientes',
      },
      {
        selector: '[data-testid="nav-vehiculos"], a[href*="vehiculos"]',
        url: '/vehiculos',
      },
      {
        selector: '[data-testid="nav-deals"], a[href*="deals"]',
        url: '/deals',
      },
      {
        selector: '[data-testid="nav-depositos"], a[href*="depositos"]',
        url: '/depositos',
      },
      {
        selector: '[data-testid="nav-inversores"], a[href*="inversores"]',
        url: '/inversores',
      },
    ]
  ...
  test('Create forms load without errors', async ({ page }) => {
    ...
    for (const createPage of createPages) {
      ...
      await expect(
        page.locator(
          'button[type="submit"], [data-testid="submit"], [data-testid="save"]'
        )
      ).toBeVisible()
    }
  })
  ...
  test('Dashboard metrics load', async ({ page }) => {
    ...
    await expect(
      page.locator(
        '[data-testid="metric"], .metric, .dashboard-card, .stat-card'
      )
    ).toBeVisible({ timeout: 10000 })
...
test.describe('Performance Smoke Tests @smoke', () => {
  ...
  test('API responses are timely', async ({ page }) => {
    ...
    const apiTimes: Array<{ url: string; status: number; time: number }> = []
    ...
    const slowApis = apiTimes.filter(
      (api: { url: string; status: number; time: number }) => api.time > 2000
    )
