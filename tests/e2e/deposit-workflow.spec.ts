...
test.describe('Deposit Workflow E2E Tests', () => {
  ...
  test('Complete deposit creation workflow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="cliente-selected"]')).toContainText('Juan Pérez García')
    ...
    await page.fill('[data-testid="vehiculo-fecha-matriculacion"]', '2020-05-15')
    ...
    await expect(page.locator('[data-testid="vehiculo-selected"]')).toContainText('BMW X5')
    
    // Step 3: Financial information
    await expect(page.locator('[data-testid="financiero-section"]')).toBeVisible()
    ...
    await expect(page.locator('[data-testid="deposito-titulo"]')).toContainText('Depósito #')
    await expect(page.locator('[data-testid="deposito-estado"]')).toContainText('ACTIVO')
    
    // Verify client and vehicle information are displayed
    await expect(page.locator('[data-testid="cliente-info"]')).toContainText('Juan Pérez García')
    await expect(page.locator('[data-testid="vehiculo-info"]')).toContainText('BMW X5')
    
    // Verify financial information
    await expect(page.locator('[data-testid="monto-info"]')).toContainText('15.000€')
    await expect(page.locator('[data-testid="dias-info"]')).toContainText('90 días')
  })
  ...
  test('Generate deposit contract workflow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="generar-contrato-deposito"]')).toBeVisible()
    await expect(page.locator('[data-testid="generar-contrato-deposito"]')).toBeEnabled()
    ...
    await expect(page.locator('[data-testid="generar-contrato-deposito"]')).toContainText('Contrato Generado')
    await expect(page.locator('[data-testid="generar-contrato-deposito"]')).toHaveClass(/bg-green/)
    
    // Verify download button appears in documentation section
    await expect(page.locator('[data-testid="descargar-contrato-deposito"]')).toBeVisible()
    await expect(page.locator('[data-testid="descargar-contrato-deposito"]')).toBeEnabled()
  ...
  test('Mark deposit as sold workflow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="marcar-vendido-toggle"]')).toBeVisible()
    ...
    await expect(page.locator('[data-testid="deposito-estado"]')).toContainText('VENDIDO')
    
    // Verify "Generar Contrato de Compra" button is now enabled
    await expect(page.locator('[data-testid="generar-contrato-compra"]')).toBeVisible()
    await expect(page.locator('[data-testid="generar-contrato-compra"]')).toBeEnabled()
  })
  ...
  test('Add notes to deposit workflow', async ({ page }) => {
    ...
    // Add a new note
    await page.fill('[data-testid="nueva-nota-textarea"]', 'Cliente llamó para consultar el estado del vehículo')
    ...
    await expect(page.locator('[data-testid="nota-item"]').first()).toContainText('Cliente llamó para consultar')
    
    // Verify note metadata
    await expect(page.locator('[data-testid="nota-fecha"]').first()).toBeVisible()
    await expect(page.locator('[data-testid="nota-usuario"]').first()).toContainText('Sistema')
    
    // Test note editing
    await page.click('[data-testid="editar-nota-btn"]')
    await page.fill('[data-testid="editar-nota-textarea"]', 'Cliente llamó para consultar el estado del vehículo - URGENTE')
    await page.click('[data-testid="guardar-nota-btn"]')
    
    // Verify note was updated
    await expect(page.locator('[data-testid="nota-item"]').first()).toContainText('URGENTE')
  })
  ...
  test('Deposit list and filters workflow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="depositos-titulo"]')).toContainText('Depósitos de venta')
    await expect(page.locator('[data-testid="nuevo-deposito-btn"]')).toBeVisible()
    
    // Verify statistics blocks
    await expect(page.locator('[data-testid="total-depositos"]')).toBeVisible()
    await expect(page.locator('[data-testid="depositos-activos"]')).toBeVisible()
    await expect(page.locator('[data-testid="depositos-finalizados"]')).toBeVisible()
    ...
    if (count > 0) {
      ...
      for (let i = 0; i < count; i++) {
        await expect(rows.nth(i).locator('[data-testid="deposito-estado"]')).toContainText('ACTIVO')
      }
    }
  ...
  test('Error handling and validation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="error-message"]')).toContainText('Debes seleccionar un cliente')
    ...
    // Verify error message
    await expect(page.locator('[data-testid="error-message"]')).toContainText('Debes seleccionar un vehículo')
    ...
    await expect(page.locator('[data-testid="validation-error"]')).toContainText('El monto debe ser positivo')
    ...
    // Verify validation error
    await expect(page.locator('[data-testid="validation-error"]')).toContainText('Los días de gestión deben ser mayor a 0')
  })
})

// Helper functions for test setup
async function setupTestDeposit(page) {
...
async function setupTestDepositWithContract(page) {
  await setupTestDeposit(page)
...
async function setupTestDepositWithContracts(page) {
  await setupTestDepositWithContract(page)
...
async function setupClientOnly(page) {
...
async function setupClientAndVehicle(page) {
  await setupClientOnly(page)
