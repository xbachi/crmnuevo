...
const createTestVehiculo = (tipo = 'C') => ({
...
const createTestDeposito = (clienteId: number, vehiculoId: number) => ({
...
describe('🔍 TESTS REALES CON BASE DE DATOS - VERIFICACIÓN COMPLETA', () => {
  ...
  describe('📋 1. VERIFICAR CREACIÓN REAL DE CLIENTE', () => {
    test('should REALLY create a client in the database', async () => {
      ...
      try {
        ...
        const verifyResponse = await fetch(`${baseUrl}/api/clientes/${createdClienteId}`)
        ...
        if (error.message.includes('fetch')) {
          console.log('⚠️  Servidor no disponible - Test marcado como PENDIENTE')
  ...
  describe('🚗 2. VERIFICAR CREACIÓN REAL DE VEHÍCULOS', () => {
    ...
    tiposVehiculo.forEach(({ tipo, nombre }) => {
      test(`should REALLY create a ${nombre} vehicle (${tipo}) in the database`, async () => {
        ...
        const vehiculoData = createTestVehiculo(tipo)
        ...
        try {
          ...
          // VERIFICAR QUE REALMENTE EXISTE EN LA BD
          const verifyResponse = await fetch(`${baseUrl}/api/vehiculos/${result.id}`)
          expect(verifyResponse.ok).toBe(true)
          ...
          console.log(`✅ VERIFICADO: Vehículo ${nombre} existe en la base de datos`)
  ...
  describe('🤝 3. VERIFICAR CREACIÓN REAL DE DEAL', () => {
    test('should REALLY create a deal in the database', async () => {
      ...
      if (!createdClienteId || createdVehiculoIds.length === 0) {
        console.log('⚠️  No hay cliente o vehículo creado - Saltando test de deal')
      ...
      try {
        ...
        // VERIFICAR QUE REALMENTE EXISTE EN LA BD
        const verifyResponse = await fetch(`${baseUrl}/api/deals/${createdDealId}`)
        expect(verifyResponse.ok).toBe(true)
  ...
  describe('📦 4. VERIFICAR CREACIÓN REAL DE DEPÓSITO', () => {
    test('should REALLY create a deposit in the database', async () => {
      ...
      if (!createdClienteId || createdVehiculoIds.length < 3) {
        console.log('⚠️  No hay cliente o vehículo de depósito - Saltando test de depósito')
        return // Skip test si no hay datos
      }

      const depositoData = createTestDeposito(createdClienteId, createdVehiculoIds[2]) // Vehículo tipo D
      ...
      try {
        ...
        // VERIFICAR QUE REALMENTE EXISTE EN LA BD
        const verifyResponse = await fetch(`${baseUrl}/api/depositos/${createdDepositoId}`)
        expect(verifyResponse.ok).toBe(true)
    ...
    test('should REALLY generate and download contracts', async () => {
      ...
      try {
        ...
        if (createdDealId) {
          ...
          const contractResponse = await fetch(`${baseUrl}/api/contratos/reserva/${createdDealId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          })

          console.log('📡 Respuesta generación contrato deal:', contractResponse.status)
        ...
        if (createdDepositoId) {
          console.log(`📄 Generando contrato para depósito ${createdDepositoId}...`)
          
          const depositContractResponse = await fetch(`${baseUrl}/api/contratos/deposito/${createdDepositoId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          })

          console.log('📡 Respuesta generación contrato depósito:', depositContractResponse.status)
          
          if (depositContractResponse.ok) {
            const contractResult = await depositContractResponse.text()
            expect(contractResult.length).toBeGreaterThan(0)
            console.log('✅ VERIFICADO: Contrato de depósito generado exitosamente')
          } else {
    ...
    test('should REALLY update vehicle states in kanban', async () => {
      ...
      try {
        ...
        for (const estado of nuevosEstados) {
          ...
          const updateResponse = await fetch(`${baseUrl}/api/vehiculos/${vehiculoId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
  ...
  afterAll(() => {
    ...
    console.log(`📊 Cliente creado: ${createdClienteId ? '✅ ID ' + createdClienteId : '❌ NO'}`)
    console.log(`📊 Vehículos creados: ${createdVehiculoIds.length > 0 ? '✅ ' + createdVehiculoIds.length + ' vehículos' : '❌ NO'}`)
    console.log(`📊 Deal creado: ${createdDealId ? '✅ ID ' + createdDealId : '❌ NO'}`)
    console.log(`📊 Depósito creado: ${createdDepositoId ? '✅ ID ' + createdDepositoId : '❌ NO'}`)
    
    if (createdClienteId || createdVehiculoIds.length > 0) {
      console.log('\n✅ ÉXITO: Se verificaron creaciones REALES en la base de datos')
      console.log('🔍 Para ver los registros creados, revisa las tablas:')
      console.log('   - Clientes: SELECT * FROM Clientes WHERE nombre LIKE \'%Test Real%\'')
      console.log('   - Vehículos: SELECT * FROM Vehiculos WHERE marca LIKE \'%Test%\'')
      console.log('   - Deals: SELECT * FROM Deals WHERE id IN (' + (createdDealId || 'ninguno') + ')')
      console.log('   - Depósitos: SELECT * FROM Depositos WHERE id IN (' + (createdDepositoId || 'ninguno') + ')')
    } else {
      console.log('\n⚠️  NOTA: Los tests requieren servidor ejecutándose en localhost:3000')
      console.log('   Para ejecutar tests reales: npm run dev (en otra terminal)')
    }
  })
})
