import { periodoFromDate, gestoriaFolder, carpetaDestino, normPlate } from '@/lib/facturasRegistro'

describe('periodoFromDate', () => {
  it('deriva año/trimestre/mes', () => {
    expect(periodoFromDate('2026-06-18')).toEqual({ anio: 2026, trimestre: 2, mes: 6 })
    expect(periodoFromDate('2026-01-05')).toEqual({ anio: 2026, trimestre: 1, mes: 1 })
    expect(periodoFromDate('2026-12-31')).toEqual({ anio: 2026, trimestre: 4, mes: 12 })
  })
  it('null si inválida', () => {
    expect(periodoFromDate('')).toEqual({ anio: null, trimestre: null, mes: null })
    expect(periodoFromDate(undefined)).toEqual({ anio: null, trimestre: null, mes: null })
  })
})

describe('gestoriaFolder', () => {
  it('arma la ruta del mes con el trimestre correcto', () => {
    expect(gestoriaFolder(2026, 6)).toBe('GESTORIA/2026/2do trimestre/Facturas/junio')
    expect(gestoriaFolder(2026, 1)).toBe('GESTORIA/2026/1re trimestre/Facturas/enero')
    expect(gestoriaFolder(2026, 10)).toBe('GESTORIA/2026/4to trimestre/Facturas/octubre')
  })
  it('null sin período', () => {
...
describe('carpetaDestino', () => {
  ...
  it('gestoria sin fecha → sin-periodo', () => {
    expect(carpetaDestino('gestoria', null, null, null).tipo).toBe('sin-periodo')
  })
})
