    ...
    async (forceRefresh = false) => {
      ...
      try {
        ...
        if (
          ...
          now - cacheRef.current.stats.timestamp < CACHE_TTL
        ) {
        ...
        if (
          !forceRefresh &&
          cacheRef.current.depositos &&
          now - cacheRef.current.depositos.timestamp < CACHE_TTL
        ) {
        ...
        if (
          !forceRefresh &&
          cacheRef.current.operaciones &&
          now - cacheRef.current.operaciones.timestamp < CACHE_TTL
        ) {
