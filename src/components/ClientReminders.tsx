...
export default function ClientReminders({
  ...
  const [formData, setFormData] = useState({
    titulo: '',
    descripcion: '',
    tipo: 'llamada' as ClientReminder['tipo'],
    prioridad: 'media' as ClientReminder['prioridad'],
    fechaRecordatorio: '',
  })
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const response = await fetch(
        `/api/clientes/${clienteId}/recordatorios/${id}`,
        {
          method: 'DELETE',
        }
      )
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      tipo: e.target.value as ClientReminder['tipo'],
                    }))
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      prioridad: e.target.value as ClientReminder['prioridad'],
                    }))
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <h4
                    className={`font-medium ${reminder.completado ? 'line-through text-gray-500' : 'text-gray-900'}`}
                  >
                    {reminder.titulo}
                  </h4>
                  ...
                  {reminder.descripcion && (
                    <p
                      className={`text-sm mt-1 ${reminder.completado ? 'text-gray-400' : 'text-gray-600'}`}
                    >
                      {reminder.descripcion}
                    </p>
                  )}
