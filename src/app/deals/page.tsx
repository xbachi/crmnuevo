  ...
  const getFilteredDealsByTime = (deals: Deal[]) => {
    ...
    switch (timeFilter) {
      ...
      case '3months':
        ...
        return deals.filter(deal => new Date(deal.createdAt) >= threeMonthsAgo)
      case '6months':
  ...
  const getFilteredDeals = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(deal => 
        ...
        deal.cliente?.apellidos?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        deal.vehiculo?.marca?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        deal.vehiculo?.modelo?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-300 text-sm">
                      {deals.length} registrados • {getFilteredDeals().length} mostrados
                    </p>
                  </div>
                </div>
                ...
                <div className="flex bg-slate-100 rounded-xl p-1">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center space-x-2 ${
                      activeTab === 'nuevo' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                    }`}
                  >
                    <span className="font-medium">Nuevo</span>
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      activeTab === 'nuevo' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'
                    }`}>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center space-x-2 ${
                      activeTab === 'reservado' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                    }`}
                  >
                    <span className="font-medium">Reservado</span>
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      activeTab === 'reservado' ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-200 text-slate-600'
                    }`}>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center space-x-2 ${
                      activeTab === 'vendido' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                    }`}
                  >
                    <span className="font-medium">Vendido</span>
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      activeTab === 'vendido' ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-600'
                    }`}>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center space-x-2 ${
                      activeTab === 'facturado' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                    }`}
                  >
                    <span className="font-medium">Facturado</span>
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      activeTab === 'facturado' ? 'bg-purple-100 text-purple-700' : 'bg-slate-200 text-slate-600'
                    }`}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-6 flex-1">
                          <div className="min-w-[140px] max-w-[140px]">
                            <div className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Deal #</div>
                          </div>
                          <div className="min-w-[140px] max-w-[140px]">
                          ...
                          <div className="min-w-[90px] max-w-[90px] text-right">
                            <div className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Total</div>
                          </div>
                          <div className="min-w-[70px] max-w-[70px] text-right">
                            <div className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Seña</div>
                          </div>
                          <div className="min-w-[90px] max-w-[90px] text-right">
                ...
                <div className="space-y-2">
                    ...
                    >
                      <div className="flex items-center">
                        <div className={`w-1 h-16 ${
                          deal.estado === 'nuevo' ? 'bg-blue-500' :
                          deal.estado === 'reservado' ? 'bg-yellow-500' :
                          deal.estado === 'vendido' ? 'bg-green-500' :
                          'bg-purple-500'
                        }`}></div>
                        <div className="flex-1 p-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-6 flex-1">
                              <div className="min-w-[140px] max-w-[140px]">
                                <div className="font-semibold text-slate-900 truncate" title={deal.numero}>
                                  #{deal.numero}
                                </div>
                              </div>
                              <div className="min-w-[140px] max-w-[140px]">
                                <div className="font-medium text-slate-900 truncate" title={`${deal.cliente?.nombre || ''} ${deal.cliente?.apellidos || ''}`.trim()}>
                                ...
                                {deal.cliente?.telefono && (
                                  <div className="text-sm text-slate-500 truncate" title={deal.cliente.telefono}>
                                    {deal.cliente.telefono}
                                  </div>
                                )}
                              </div>
                              <div className="min-w-[160px] max-w-[160px]">
                                <div className="font-medium text-slate-900 truncate" title={`${deal.vehiculo?.marca || ''} ${deal.vehiculo?.modelo || ''}`.trim()}>
                                  {deal.vehiculo?.marca && deal.vehiculo?.modelo
                                    ? `${deal.vehiculo.marca} ${deal.vehiculo.modelo}`.trim()
                                    : 'Sin vehículo'
                                  }
                                </div>
                                <div className="text-sm text-slate-500 truncate" title={deal.vehiculo?.matricula || ''}>
                                  {deal.vehiculo?.matricula || '-'}
                                </div>
                              </div>
                              <div className="min-w-[90px] max-w-[90px] text-right">
                                <div className="font-semibold text-slate-900 truncate">
                                  {deal.importeTotal ? formatCurrency(deal.importeTotal) : '-'}
                                </div>
                              </div>
                              <div className="min-w-[70px] max-w-[70px] text-right">
                                <div className="font-medium text-slate-900 truncate">
                                  {deal.importeSena ? formatCurrency(deal.importeSena) : '-'}
                                </div>
                              </div>
                              <div className="min-w-[90px] max-w-[90px] text-right">
                              ...
                              <div className="min-w-[80px] max-w-[80px] text-center">
                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                  deal.estado === 'nuevo' ? 'bg-blue-100 text-blue-700' :
                                  deal.estado === 'reservado' ? 'bg-yellow-100 text-yellow-700' :
                                  deal.estado === 'vendido' ? 'bg-green-100 text-green-700' :
                  ...
                  {getFilteredDeals().length === 0 && (
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-medium text-slate-900 mb-1">No hay deals</h3>
                      <p className="text-slate-500">No se encontraron deals en el estado "{activeTab}"</p>
                    </div>
                  )}
                </div>
              </div>
