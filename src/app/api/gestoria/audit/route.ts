  ...
  try {
    ...
    const sinFecha = await pool.query(
      `SELECT id, proveedor, numero_factura, nombre_archivo FROM facturas_registro
        WHERE ${where} AND mes IS NULL ORDER BY proveedor`,
      params
    )
    const sinNumero = await pool.query(
      `SELECT id, proveedor, mes, nombre_archivo FROM facturas_registro
        WHERE ${where} AND (numero_factura IS NULL OR numero_factura = '') ORDER BY proveedor`,
      params
    )
    const cocheSinMat = await pool.query(
      `SELECT id, proveedor, categoria, nombre_archivo FROM facturas_registro
        WHERE ${where} AND categoria LIKE 'coche-%' AND (matricula IS NULL OR matricula = '') ORDER BY proveedor`,
      params
    )
    ...
    return NextResponse.json({
      ...
      porMes: porMes.rows.map((r) => ({
        mes: r.mes,
        nombre: r.mes ? MES_NOM[r.mes - 1] : null,
        n: r.n,
        total: r.total,
      })),
