...
export async function PUT(
  ...
  try {
    ...
    if (dias_gestion) {
      ...
      const depositoActual = await pool.query(
        'SELECT fecha_inicio FROM depositos WHERE id = $1',
        [id]
      )
    ...
    if (Object.keys(body).length === 1 && body.estado) {
      ...
      console.log(
        `✅ Query simple ejecutado, filas afectadas: ${result.rows.length}`
      )
    ...
    console.log(
      `✅ Query completo ejecutado, filas afectadas: ${result.rows.length}`
    )
