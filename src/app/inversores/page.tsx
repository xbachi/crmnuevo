...
export default function InversoresPage() {
  ...
  const handleView = (inversor: Inversor) => {
    const nombreSlug = inversor.nombre.toLowerCase()
      .replace(/[^a-z0-9\s]/g, '') // Remover caracteres especiales
      .replace(/\s+/g, '-') // Reemplazar espacios con guiones
      .trim()
    router.push(`/inversores/${inversor.id}-${nombreSlug}`)
  }
            ...
            <div className="px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h1 className="text-xl font-bold text-white">Inversores</h1>
                    <p className="text-slate-300 text-sm">
                      {inversores.length} registrados • {filteredInversores.length} mostrados
                    </p>
                  </div>
                </div>
