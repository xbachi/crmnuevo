...
export default function ClientesPage() {
  ...
  const [sortBy, setSortBy] = useState<'fecha' | 'nombre' | 'prioridad'>(
    'fecha'
  )
  ...
  const filteredClientes = clientes
    .filter((cliente) => {
      ...
      const matchesPrioridad =
        !prioridadFilter || (cliente.prioridad || 'media') === prioridadFilter
      ...
      const matchesPrecioMax =
        ...
        (clientePrecio > 0 &&
          precioMaxValue > 0 &&
          clientePrecio >= precioMaxValue)

      // Debug log para verificar el filtro
      if (precioMaxFilter && clientePrecio > 0) {
        console.log(
          `Cliente: ${cliente.nombre}, Precio cliente: ${clientePrecio}, Filtro: ${precioMaxValue}, Match: ${matchesPrecioMax}`
        )
      }

      const matchesKilometraje =
        !kilometrajeFilter ||
        ((cliente.intereses?.kilometrajeMaximo || 0) > 0 &&
          parseInt(kilometrajeFilter) > 0 &&
          (cliente.intereses?.kilometrajeMaximo || 0) >=
            parseInt(kilometrajeFilter))

      const matchesAñoMin =
        !añoMinFilter ||
        ((cliente.intereses?.añoMinimo || 0) > 0 &&
          parseInt(añoMinFilter) > 0 &&
          (cliente.intereses?.añoMinimo || 0) <= parseInt(añoMinFilter))

      const matchesCombustible =
        !combustibleFilter ||
        cliente.intereses?.combustiblePreferido === combustibleFilter ||
        (combustibleFilter === 'cualquiera' &&
          cliente.intereses?.combustiblePreferido === 'cualquiera')

      const matchesCambio =
        !cambioFilter ||
        cliente.intereses?.cambioPreferido === cambioFilter ||
        (cambioFilter === 'cualquiera' &&
          cliente.intereses?.cambioPreferido === 'cualquiera')
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        case 'fecha':
          ...
          break
        case 'nombre':
          comparison = `${a.nombre} ${a.apellidos}`.localeCompare(
        ...
        case 'prioridad':
          ...
          comparison =
            ...
            prioridadOrder[b.prioridad as keyof typeof prioridadOrder]
          break
      }
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          <div className="mb-8">
            ...
            <p className="text-slate-600">
              Registra y haz seguimiento de todos los clientes
            </p>
          </div>
          <LoadingSkeleton />
        </main>
      ...
      <div className="min-h-full bg-gradient-to-br from-slate-50 via-orange-50 to-red-100">
        <main className="w-[80%] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
              ...
              {showAdvancedFilters && (
                <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200/60 p-4">
                  <h3 className="text-sm font-medium text-slate-700 mb-4">
                    🔍 Filtros por Intereses del Cliente
                  </h3>
                ...
                >
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <div className="flex items-center justify-between">
                      <span>Estado:</span>
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${getEstadoColor(cliente.estado || 'nuevo')}`}
                      >
          ...
          <ConfirmDeleteModal
            ...
            title="Eliminar cliente"
            message="¿Estás seguro de eliminar este cliente? También se borrarán sus notas y recordatorios. Esta acción no se puede deshacer."
            fileName={
              clienteToDelete
                ? `${capitalizeText(clienteToDelete.nombre)} ${capitalizeText(clienteToDelete.apellidos)}`.trim()
                : undefined
            }
