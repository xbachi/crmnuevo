...
const COSTOBENEFICIO_SPREADSHEET_ID_DEFAULT =
...
async function loadVehicleCosts(
  dealId?: number | null,
  vehiculoId?: number | null
): Promise<VehicleCostData | null> {
  if (dealId == null && vehiculoId == null) return null
  const res =
    vehiculoId != null
      ? await pool.query(
          `SELECT referencia, marca, modelo, matricula,
                "precioCompra", "gastosTransporte", "gastosMecanica",
                "gastosPintura", "gastosLimpieza", "gastosOtros"
...
export function buildCarRow(
  ...
  const b = ''
  return [
    toEsDate(opts.invoiceDate),
    MES_FORMULA(r),
    v.referencia ?? b,
    [v.marca, v.modelo].filter(Boolean).join(' ') || b,
    v.matricula ?? b,
    regimen,
    compra ?? b,
    b,
...
async function openTab(sheetName?: string): Promise<Ctx | { error: string }> {
  ...
  const wanted =
    sheetName ||
    process.env.COSTOBENEFICIO_SHEET_NAME ||
    COSTOBENEFICIO_SHEET_NAME_DEFAULT
  const meta = await api.spreadsheets.get({
    spreadsheetId,
    fields: 'sheets(properties(sheetId,title))',
  })
  const tab = (meta.data.sheets ?? []).find(
    (s) => s.properties?.title === wanted
  )
  ...
  const vr = await api.spreadsheets.values.get({
    spreadsheetId,
    range: `${tab.properties.title}!A1:S`,
  ...
  try {
    if (process.env.COSTOBENEFICIO_DISABLED === '1')
      return {
        ok: true,
        action: 'skipped',
        detail: 'COSTOBENEFICIO_DISABLED=1',
        warnings,
      }
    if (
      ...
      return {
        ok: true,
        action: 'skipped',
        detail: 'faltan credenciales Google',
        warnings,
      }
    }
    const v = await loadVehicleCosts(opts.dealId, opts.vehiculoId)
    if (!v)
      return {
        ok: false,
        action: 'error',
        detail: 'Factura sin vehículo asociado',
        warnings,
      }
    ...
    if (v.gastosLimpieza == null)
      warnings.push('limpieza (gastosLimpieza) vacío')
    ...
    if (mesIdx < 0)
      return {
        ok: false,
        action: 'error',
        detail: `fecha inválida: ${opts.invoiceDate}`,
        warnings,
      }
      ...
      requests.push({
        insertDimension: {
          range: {
            sheetId,
            dimension: 'ROWS',
            startIndex: bandRow - 1,
            endIndex: bandRow - 1 + 3,
          },
    ...
    const plan = {
      ...
      mes,
      filaDestino: targetRow,
      bandaNueva: createBand,
      valores: {
        A_fecha: toEsDate(opts.invoiceDate),
        C_ref: v.referencia,
        D_coche: [v.marca, v.modelo].filter(Boolean).join(' '),
        E_matricula: v.matricula,
        F_regimen: regimen,
        G_compra: compra,
        'desglose(compra+porte)': {
          precioCompra: v.precioCompra,
          transporte: v.gastosTransporte,
        },
        I_cn: CN_INFORME,
    ...
    if (opts.dryRun) {
      ...
      return {
        ok: true,
        action: 'dry-run',
        detail: `insertaría fila ${targetRow} (${mes}) en "${sheetTitle}"${createBand ? ' [crea banda]' : ''}`,
        warnings,
        plan,
      }
    }
    ...
    const data: sheets_v4.Schema$ValueRange[] = [
      { range: `${sheetTitle}!A${targetRow}:S${targetRow}`, values: [carRow] },
    ]
    if (createBand) {
      data.push({ range: `${sheetTitle}!A${bandRow}`, values: [[mes]] })
      data.push({
        range: `${sheetTitle}!D${targetRow + 1}:R${targetRow + 1}`,
        values: [
          [
            `TOTAL ${mes}`,
            '',
    ...
    log(
      `fila ${targetRow} (${mes}) en "${sheetTitle}": ${v.referencia ?? '?'} / ${v.matricula ?? '?'} / ${opts.salePrice}${createBand ? ' [banda nueva]' : ''}${warnings.length ? ` | avisos: ${warnings.join(' | ')}` : ''}`
    )
    return {
      ok: true,
      action: 'inserted',
      detail: `fila ${targetRow} (${mes}) en "${sheetTitle}"`,
      warnings,
      plan,
    }
  } catch (err) {
...
export async function notifyCostoBeneficio(
  opts: CostoBeneficioOptions
): Promise<void> {
  const startTime = Date.now()
  let result: CostoBeneficioResult = {
    ok: false,
    action: 'error',
    detail: 'no result',
    warnings: [],
 ...
 */
export async function resyncVehiculoRowToCB(
  vehiculoId: number,
  sheetName?: string
): Promise<CostoBeneficioResult> {
  const warnings: string[] = []
  try {
    ...
    if (!r)
      return {
        ok: false,
        action: 'error',
        detail: `Vehiculo ${vehiculoId} no existe`,
        warnings,
      }
    const num = (x: unknown) => (x != null ? Number(x) : null)
    const costs: CbCosts = {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if ((plate && rp && plate === rp) || (ref && rr && ref === rr)) {
        rowNum = i + 1
        break
      }
    }
    if (!rowNum)
      return {
        ok: true,
        action: 'skipped',
        detail: 'coche sin fila en CB (aún)',
        warnings,
      }

    const updates = reconcileCostCells(
      rows[rowNum - 1] as string[],
      costs,
      'overwrite'
    )
    if (updates.length === 0)
      return { ok: true, action: 'skipped', detail: 'CB ya coincide', warnings }

    const data = updates.map((u) => ({
      range: `${sheetTitle}!${u.col}${rowNum}`,
      values: [[u.value]],
    }))
    await api.spreadsheets.values.batchUpdate({
      spreadsheetId,
    ...
    log(
      `resync fila ${rowNum} (${r.matricula}): ${updates.map((u) => `${u.col}=${u.value}`).join(', ')}`
    )
    return {
      ok: true,
      action: 'inserted',
      detail: `fila ${rowNum}: ${updates.map((u) => u.col).join(',')}`,
      warnings,
    }
  } catch (err) {
    const msg = (err as Error)?.message ?? String(err)
