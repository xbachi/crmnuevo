...
export interface InteresesCliente {
  ...
  añoMinimo: number
  combustiblePreferido: 'diesel' | 'gasolina' | 'hibrido' | 'electrico' | 'cualquiera'
  cambioPreferido: 'manual' | 'automatico' | 'cualquiera'
  coloresDeseados: string[]
  necesidadesEspeciales: string[]
  formaPagoPreferida: 'financiacion' | 'contado' | 'entrega_usado' | 'cualquiera'
}
...
const NOTAS_CLIENTES_DB_PATH = path.join(process.cwd(), 'data', 'notas_clientes.json')
...
export async function checkUniqueFields(referencia: string, matricula: string, bastidor: string, excludeId?: number): Promise<{ field: string; exists: boolean } | null> {
  const vehiculos = await getVehiculos()
  
  const existing = vehiculos.find(v => 
    v.id !== excludeId && (
  ...
  if (existing.referencia === referencia) return { field: 'referencia', exists: true }
  if (existing.matricula === matricula) return { field: 'matrícula', exists: true }
...
export async function updateVehiculo(id: number, data: Omit<Vehiculo, 'id' | 'createdAt' | 'updatedAt'>) {
  await ensureDataDir()
...
// Guardar inversor
export async function saveInversor(inversorData: Omit<Inversor, 'id' | 'createdAt' | 'updatedAt'>): Promise<Inversor> {
  await ensureDataDir()
  
  const inversores = await getInversores()
  const newId = inversores.length > 0 ? Math.max(...inversores.map(i => i.id)) + 1 : 1
  ...
  await fs.writeFile(INVERSORES_DB_PATH, JSON.stringify(inversores, null, 2), 'utf8')
  
  return newInversor
}

// Actualizar inversor
export async function updateInversor(id: number, data: Omit<Inversor, 'id' | 'createdAt' | 'updatedAt'>) {
  await ensureDataDir()
...
export async function deleteInversor(id: number) {
  ...
  await fs.writeFile(INVERSORES_DB_PATH, JSON.stringify(inversoresFiltrados, null, 2))
...
export async function getVehiculosByInversor(inversorId: number): Promise<Vehiculo[]> {
  const vehiculos = await getVehiculos()
  return vehiculos.filter(v => v.esCocheInversor && v.inversorId === inversorId)
}

// Calcular métricas de inversor
export async function getInversorMetrics(inversorId: number, periodo?: { desde: string; hasta: string }) {
  ...
  if (periodo) {
    vehiculosFiltrados = vehiculos.filter(v => {
      const fechaCompra = v.fechaCompra ? new Date(v.fechaCompra) : new Date(v.createdAt)
      return fechaCompra >= new Date(periodo.desde) && fechaCompra <= new Date(periodo.hasta)
    })
  }
  ...
  const capitalInvertido = inversor.capitalInvertido || capitalInvertidoActual || 0
  const capitalDisponible = (capitalAportado || 0) - (capitalInvertido || 0)
  
  const roi = capitalAportado > 0 ? (beneficioAcumulado / capitalAportado) * 100 : 0
  ...
  return {
    ...
    diasPromedioEnStock: enStock.length > 0 ? 
      enStock.reduce((sum, v) => {
        const dias = Math.floor((Date.now() - new Date(v.createdAt).getTime()) / (1000 * 60 * 60 * 24))
        return sum + dias
      }, 0) / enStock.length : 0
  }
}
...
// Guardar cliente
export async function saveCliente(cliente: Omit<Cliente, 'id' | 'createdAt' | 'updatedAt'>): Promise<Cliente> {
  await ensureDataDir()
  const clientes = await getClientes()
...
// Actualizar cliente
export async function updateCliente(id: number, updates: Partial<Omit<Cliente, 'id' | 'createdAt'>>): Promise<Cliente> {
  const clientes = await getClientes()
...
export async function deleteCliente(id: number): Promise<void> {
  ...
  await fs.writeFile(CLIENTES_DB_PATH, JSON.stringify(filteredClientes, null, 2))
}

// ===== FUNCIONES PARA NOTAS DE CLIENTES =====

// Obtener todas las notas de un cliente
export async function getNotasByCliente(clienteId: number): Promise<NotaCliente[]> {
  await ensureDataDir()
  try {
    const data = await fs.readFile(NOTAS_CLIENTES_DB_PATH, 'utf-8')
    const notas: NotaCliente[] = JSON.parse(data)
    return notas.filter(n => n.clienteId === clienteId).sort((a, b) => 
...
export async function addNotaCliente(nota: Omit<NotaCliente, 'id' | 'createdAt'>): Promise<NotaCliente> {
  await ensureDataDir()
...
export async function buscarClientesPorVehiculo(criterios: {
  ...
  return clientes.filter(cliente => {
    ...
    if (criterios.marca && !intereses.vehiculoPrincipal.toLowerCase().includes(criterios.marca.toLowerCase()) &&
        !intereses.modelosAlternativos.some(m => m.toLowerCase().includes(criterios.marca!.toLowerCase()))) {
      return false
    }
    
    if (criterios.modelo && !intereses.vehiculoPrincipal.toLowerCase().includes(criterios.modelo.toLowerCase()) &&
        !intereses.modelosAlternativos.some(m => m.toLowerCase().includes(criterios.modelo!.toLowerCase()))) {
      return false
    }
    
    // Filtro por precio
    if (criterios.precioMaximo && intereses.precioMaximo > criterios.precioMaximo) {
      return false
    }
    
    // Filtro por kilometraje
    if (criterios.kilometrajeMaximo && intereses.kilometrajeMaximo > criterios.kilometrajeMaximo) {
      return false
    }
    ...
    if (criterios.combustible && intereses.combustiblePreferido !== 'cualquiera' && 
        intereses.combustiblePreferido !== criterios.combustible) {
      return false
    }
    
    // Filtro por cambio
    if (criterios.cambio && intereses.cambioPreferido !== 'cualquiera' && 
